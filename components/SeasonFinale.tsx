...
export function SeasonFinale({ season }: SeasonFinaleProps) {
  ...
  return (
    <div
      ...
      style={{ borderTopColor: userWon ? "#eab308" : (userUniverse?.accentColor ?? "#64748b") }}
    >
      <p className="broadcast-label mb-1">Season {season.seasonNumber} — Final Standings</p>
      <h2 className="font-display text-2xl font-bold uppercase tracking-wide text-broadcast-highlight md:text-3xl">
        {userWon ? "Champions!" : `Finished ${ordinal(position)}`}
      </h2>
      ...
      {userWon ? (
        ...
        <p className="mt-4 text-sm text-slate-400">
          {championUni?.name ?? "The league"} lift the trophy. Better luck next season.
        </p>
      )}
